...
local require_plugin = function(p)
    if Modx.plugins[p] ~= nil and not Modx.plugins[p].enabled then
        return nil
    end
...
local add_which_key = function(starter_key, definitions)
    Modx.plugins.which_key.user_defined[starter_key] = definitions or {}
end
...
Modx.utils = {
        ...
        copy = function(t1, t2)
            for k, v in pairs(t2) do
                if (type(v) == "table") and (type(t1[k] or false) == "table") then
                    Modx.utils.tables.copy(t1[k], t2[k])
                else
                    t1[k] = v
                end
            end
    ...
    plugins = {
        ...
        add_user = function(p)
            table.insert(Modx.plugins.user, p)
        end,
        ...
        -- to the Modx.packages.user table for installation.
        --
        ...
        require_if_installed = function(mod_name, pkg, conf)
            ...
            elseif pkg_is_git then
                table.insert(Modx.plugins.user, {packer_ref, conf})
                return nil
            end
        end
    }
